...
const COLUMNS = [
  ...
  { id: "usd" as const, label: "USD equivalent" },
  { id: "actions" as const, label: "" },
];

type ForeignCurrencyAccountListProps = {
  accounts: ForeignCurrencyAccount[];
};

export function ForeignCurrencyAccountList({
  accounts,
}: ForeignCurrencyAccountListProps) {
  return (
    <section>
      <h2 className="text-lg font-semibold text-primary mb-4">Your accounts</h2>
      <TableCard.Root size="md">
        <Table aria-label="Foreign currency accounts" selectionMode="none">
